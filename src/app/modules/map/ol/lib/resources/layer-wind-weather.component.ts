...
export class LayerWindWeatherComponent implements OnChanges, OnDestroy {
  @Input() show = false;
  @Input() opacity = 0.55;
  @Input() indicator: 'arrow' | 'barb' = 'barb';
  ...
  // Wind speed shown for both indicators — the label is always displayed.
  private speedLabel(speedMs: number, offsetY: number) {
    const speedKn = Math.round(Convert.transform(speedMs, 'm/s', 'kn'));
    return new Text({
      text: `${speedKn} kn`,
      offsetY,
